...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadDir = req.body.uploadDir || 'dynamicFolder'; // Fallback to 'uploads' if no directory is specified
...
exports.uploadImage = (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      const uploadDir = req.body.uploadDir || 'dynamicFolder'; // Directory used for this upload
